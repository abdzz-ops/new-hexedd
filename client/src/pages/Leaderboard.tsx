...
import { Eye, Crown, Trophy, Zap, TrendingUp } from "lucide-react";
...
const TABS = [
  ...
  { key: "rising", label: "Rising", desc: "Growing fast (last 12h)", icon: TrendingUp, queryKey: "/api/public/rising" },
];
    ...
    <div className="min-h-screen bg-[#050505] text-white">
      ...
      <main className="max-w-3xl mx-auto px-4 py-12">
        <div className="mb-10 text-center">
          ...
          <p className="text-gray-500 text-sm mt-2">{isRising ? "Rising profiles growing fast in the last 12 hours" : "Top profiles by views"}</p>
        </div>

        <div className="flex justify-center mb-8">
          <div className="flex gap-1 p-1 bg-[#0e0e0e] border border-white/[0.06] rounded-2xl flex-wrap justify-center">
            {TABS.map(t => (
              <button
                key={t.key}
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {isRising && (
              <div className="mb-4 p-3 bg-orange-500/5 border border-orange-500/20 rounded-xl text-center">
                <p className="text-[11px] text-orange-400/80">Small profiles gaining views fast · refreshes every 12 hours</p>
              </div>
            )}
            ...
            {isLoading ? (
              ...
              <div className="text-center py-24">
                <p className="text-5xl mb-4">{isRising ? "📈" : "🏆"}</p>
                <p className="text-gray-500 font-bold">{isRising ? "No rising profiles yet..." : "Seems empty here..."}</p>
                <p className="text-gray-600 text-sm mt-1">{isRising ? "Check back in a few hours!" : "Be the first to get views!"}</p>
              </div>
            ) : (
              <div className="space-y-3">
                {entries.map((entry, index) => {
                  const rankColor = isRising ? "#f97316" : (RANK_COLORS[index] ?? "#555");
                  const isTop3 = !isRising && index < 3;
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          ...
                          {isRising && <span className="text-[8px] px-1.5 py-0.5 rounded bg-orange-500/20 text-orange-400 font-black uppercase">Rising</span>}
                        </div>
                        <span className="text-[10px] text-gray-600">@{entry.username}</span>
                      </div>

                      <div className="flex items-center gap-1.5 shrink-0">
                        {isRising && entry.recentViews ? (
                          <div className="text-right">
                            <div className="flex items-center gap-1">
                              <TrendingUp className="w-3 h-3 text-orange-500" />
                              <span className="text-sm font-black text-orange-500">+{entry.recentViews}</span>
                            </div>
                            <span className="text-[9px] text-gray-600">{entry.views} total</span>
                          </div>
                        ) : (
                          <>
        ...
        {entries && entries.length > 0 && (
          <p className="text-center text-[10px] text-gray-700 mt-8 uppercase font-bold tracking-widest">{tab.desc} · {isRising ? "Updated every 12h" : `Top ${entries.length}`}</p>
        )}
      </main>
    </div>
  );
