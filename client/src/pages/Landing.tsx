...
export default function Landing() {
  const { user } = useAuth();
  const { data: members } = useQuery<any[]>({ queryKey: ["/api/public/members"] });
  ...
  return (
    ...
    >
      ...
      {/* ─── Hero ──────────────────────────────────────────────────── */}
      <section className="relative pt-48 pb-12 px-6 flex flex-col items-center text-center overflow-hidden">
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[900px] h-[600px] opacity-20" style={{ background: "radial-gradient(ellipse at 50% 0%, #F97316 0%, transparent 60%)" }} />
          <div className="absolute top-20 left-1/4 w-[500px] h-[500px] rounded-full opacity-5" style={{ background: "radial-gradient(circle, #9333ea, transparent 70%)" }} />
          <div className="absolute top-20 right-1/4 w-[400px] h-[400px] rounded-full opacity-5" style={{ background: "radial-gradient(circle, #3b82f6, transparent 70%)" }} />
        </div>
        ...
        >
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-orange-500/25 bg-orange-500/8 text-[11px] font-semibold text-orange-400 tracking-wide mb-10">
            <span className="w-1.5 h-1.5 rounded-full bg-orange-400 animate-pulse" />
            Free to use · No credit card
          </div>

          <h1
            className="text-6xl sm:text-7xl md:text-8xl font-extrabold leading-[0.92] mb-8 tracking-tight"
            style={{ fontFamily: "var(--font-display)" }}
          >
            The #1 bio page
            <br />
          ...
          <p className="text-gray-400 text-lg md:text-xl max-w-2xl mx-auto mb-8 leading-relaxed font-normal">
            The Top #1 Bio Page. Get it set up in 5 seconds straight.
          </p>

          <div className="flex flex-wrap items-center justify-center gap-2 mb-10">
            {["Upload your Music", "Make it  as you like", "Take the tijme", "Gain Badges", "Custom Effects"].map(f => (
              <span key={f} className="px-3 py-1 rounded-full bg-white/[0.04] border border-white/[0.08] text-[11px] font-semibold text-gray-400">{f}</span>
            ))}
          </div>
            ...
            <AnimatePresence>
              {buttonsVisible && (
                <motion.a
                  ...
                  transition={{ duration: 0.4 }}
                  className="group flex items-center gap-2.5 px-8 py-4 bg-orange-500 text-white font-bold rounded-2xl hover:bg-white hover:text-black transition-all text-sm"
                >
                  Login
      ...
      <section className="py-14 overflow-hidden">
        <div className="text-center mb-8">
          <p className="text-[11px] font-semibold uppercase tracking-widest text-orange-500/70 mb-2">Community</p>
          <h2 className="text-3xl font-extrabold" style={{ fontFamily: "var(--font-display)" }}>Meet the community</h2>
        </div>
        {members && members.length > 0 ? (
          <MembersMarquee members={members} />
        ) : (
          <div className="flex justify-center items-center py-8">
            <p className="text-gray-600 text-sm">Be the first to join Hexed.</p>
          </div>
        )}
      </section>
      ...
      <section className="py-20 px-6 border-y border-white/[0.04]">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              { step: "01", title: "Make an account", desc: "Free signup, takes about 10 seconds. Seriously." },
              { step: "02", title: "Set it up", desc: "Drop in your avatar, bio, links, pick a color. Make it yours." },
              { step: "03", title: "Share it", desc: "Copy your hexed.at/username link and paste it literally everywhere." },
            ].map(s => (
              <div key={s.step} className="relative p-6 rounded-2xl border border-white/[0.06]" style={{ background: "rgba(10,10,10,0.8)" }}>
                <p className="text-5xl font-extrabold text-white/5 mb-4 select-none" style={{ fontFamily: "var(--font-display)" }}>{s.step}</p>
        ...
        <div className="max-w-3xl mx-auto">
          <div className="relative overflow-hidden rounded-3xl border border-orange-500/20 p-10 md:p-14 text-center" style={{ background: "linear-gradient(135deg, rgba(249,115,22,0.06) 0%, rgba(0,0,0,0) 60%)" }}>
            ...
            <p className="text-gray-400 text-base mb-8 max-w-md mx-auto leading-relaxed">
              No watermark, more tracks, more effects, and a Premium badge that actually means something.
            </p>
            <div className="flex items-baseline justify-center gap-2 mb-8">
              <span className="text-6xl font-extrabold text-orange-500" style={{ fontFamily: "var(--font-display)" }}>€4.99</span>
