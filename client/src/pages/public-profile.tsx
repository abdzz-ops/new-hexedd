...
function getSpotifyEmbedUrl(url: string): string {
  const match = url.match(/open\.spotify\.com(?:\/intl-[a-z-]+)?\/(\w+)\/([A-Za-z0-9]+)/);
  if (!match) return url;
  const [, type, id] = match;
  const si = url.match(/[?&]si=([^&]+)/)?.[1];
  return `https://open.spotify.com/embed/${type}/${id}${si ? `?utm_source=oembed&si=${si}` : ""}`;
}
...
export default function PublicProfile() {
  ...
  const showLikeCounter    = getS(s, "showLikeCounter", false);
  ...
  if (profile?.backgroundVideoUrl && !isYouTubeUrl(profile.backgroundVideoUrl) && tracks.length === 0) {
    tracks.push({ title: "Background Audio", url: profile.backgroundVideoUrl, artistProfile: "" });
  }
  ...
  return (
    <div
      ...
      style={{ backgroundColor: "#050505", color: primaryText, cursor: profile?.customCursorUrl ? `url(${profile.customCursorUrl}), auto` : cursor, fontFamily: profileFont !== "inherit" ? profileFont : undefined }}
    >
      <style>{PROFILE_STYLES}</style>

      {/* Background Media */}
      {profile?.backgroundVideoUrl ? (() => {
        const bgVidUrl = profile.backgroundVideoUrl.replace(/&amp;/g, "&");
        const isYT = isYouTubeUrl(bgVidUrl);
        if (isYT) {
          const ytId = getYouTubeVideoId(bgVidUrl) || "";
          return (
            <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
              <iframe
                ...
                className="absolute"
                style={{ top: "50%", left: "50%", transform: "translate(-50%,-50%)", width: "177.78vh", height: "100vh", minWidth: "100vw", opacity: bgOpacity, filter: bgBlur > 0 ? `blur(${bgBlur}px)` : undefined, border: "none", pointerEvents: "none" }}
              />
            </div>
          );
        }
        return (
          <div className="fixed inset-0 z-0 pointer-events-none">
            <video src={res(bgVidUrl)} autoPlay loop muted playsInline
          ...
          >
            {profile?.backgroundVideoUrl ? (
              <div className="absolute inset-0 pointer-events-none">
                <video src={res(profile.backgroundVideoUrl.replace(/&amp;/g, "&"))} autoPlay loop muted playsInline
