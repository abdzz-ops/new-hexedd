...
function DiscordProfileWidget({ user, themeColor, contentAlign }: { user: any; themeColor: string; contentAlign: string }) {
  ...
  const listeningToSpotify = lanyard?.listening_to_spotify && spotify;

  const activity = lanyard?.activities?.find((a: any) => a.type === 0 && a.name !== "Spotify");
...
export default function PublicProfile() {
  ...
  const boxBlurEnabled               = getS(s, "boxBlurEnabled", true);
            ...
            >
              ...
              <div className={`relative ${profile?.bannerUrl ? "mt-28" : ""}`}>
                ...
                {showDiscordWidget && user?.discordId && discordWidgetPos === "above-bio" && (
                  <DiscordProfileWidget user={user} themeColor={themeColor} contentAlign={contentAlign} />
                )}
